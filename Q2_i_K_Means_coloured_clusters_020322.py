...
print("For k=4 plotting the colourful clusters for 5 different initialisations.\n")
...
for rand_init in range (0, 5):
    ...
    k_means_centroid_cordinates_new, k_means_class, sample_class_dict, sample_class_dict_2 = Get_K_Means_Centroid_Cordinates(k_means)
    ...
    for (x1,v1),(x2,v2) in zip(sample_class_dict.items(),sample_class_dict_2.items()):
        ...
        for c in k_means_class:#start with every class
            if v1 == c:
                ...
                plt.xlabel('x1')
                plt.ylabel('x2')
                plt.title("Coloured K means (k=4) clusters for rand. init. no.: " + str(rand_init+1))
...
Centroid Coordinates for k = 4 : [[-4.402206505376343, -3.7192917736559137], [1.0538704630390143, 1.1091417117043123], [3.4571089347826076, -4.127362956284155], [-2.311413580419581, 6.3647169014084515]] 
